...
// Obtener reservas por empleado específico
exports.obtenerReservasPorEmpleado = async (req, res) => {
    const { id_empleado } = req.params; // Se recibe el ID del empleado desde la ruta

    try {
        // Validar parámetro obligatorio
        if (!id_empleado) {
            return res.status(400).json({ message: 'El id_empleado es obligatorio.' });
        }

        // Buscar todas las reservas asociadas al empleado
        const reservas = await Reserva.findAll({
            where: {
        ...
        res.status(500).json({ error: 'Error interno al obtener reservas del empleado.' });
    }
};